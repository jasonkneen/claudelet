...
class InferenceEngine extends events_1.EventEmitter {
    constructor() {
        ...
        this.applyMutex = Promise.resolve();
    }
    /**
    ...
    async apply(originalCode, updateSnippet) {
        return this.withApplyLock(async () => {
            ...
            try {
                // Build the prompt
                const prompt = (0, config_1.buildPrompt)(originalCode, updateSnippet);
                ...
                const responsePromise = session.prompt(prompt, {
                    maxTokens: config_1.MAX_TOKENS,
                    temperature: config_1.TEMPERATURE,
                });
                ...
                const code = (0, config_1.parseOutput)(output);
    ...
    async withApplyLock(fn) {
        ...
        this.applyMutex = new Promise((resolve) => {
            release = resolve;
        });
        await previous;
        try {
    ...
    async runWithTimeout(promise) {
        ...
        try {
            const timeoutPromise = new Promise((_, reject) => {
                timer = setTimeout(() => reject(new Error('Inference timeout')), config_1.INFERENCE_TIMEOUT);
            });
