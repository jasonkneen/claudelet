...
class Downloader extends events_1.EventEmitter {
     ...
     */
    getModelPath(variant) {
        const modelDef = config_1.MODELS[variant];
        return path.join(this.storageDir, modelDef.file);
    }
    /**
     * Check if a model is already downloaded
     */
    async isDownloaded(variant) {
        const modelPath = this.getModelPath(variant);
        try {
            await fs.promises.access(modelPath, fs.constants.R_OK);
            // Verify file size matches expected
            const stats = await fs.promises.stat(modelPath);
            const expectedSize = config_1.MODELS[variant].size * 1024 * 1024; // Convert MB to bytes
    ...
    async getDownloadedModels() {
        const downloaded = [];
        for (const variant of Object.keys(config_1.MODELS)) {
    ...
    async download(variant) {
        ...
        const url = `https://huggingface.co/${config_1.MODEL_REPO}/resolve/main/${modelDef.file}`;
     ...
     */
    async downloadFile(url, destPath, variant, signal) {
