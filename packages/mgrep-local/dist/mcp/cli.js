...
function parseArgs() {
    ...
    const envOpenaiModel = process.env.MGREP_OPENAI_MODEL;
...
// File Discovery
// =============================================================================
function discoverFiles(directory, verbose) {
    const files = [];
    function walk(dir) {
        let entries;
        try {
...
function logProgress(current, total, file) {
...
// =============================================================================
async function runIndexSingle(config) {
...
// =============================================================================
async function runIndexSharded(config) {
    const startTime = Date.now();
    ...
    const getWorkerCount = () => {
        if (config.openai)
            return Math.min(shardCount, 8); // OpenAI can handle many parallel workers
        if (config.gpu)
            return Math.min(shardCount, 2); // Metal GPU limitation
...
// Commands
// =============================================================================
async function runIndex(config) {
...
async function runServe(config) {
    if (config.verbose) {
