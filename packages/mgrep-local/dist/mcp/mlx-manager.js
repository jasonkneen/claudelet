 ...
 */
export function isMlxInstalled(installDir = MLX_HOME) {
 ...
 */
export function getMlxServerDir(installDir = MLX_HOME) {
    return join(installDir, 'qwen3-embeddings-mlx');
}
/**
 * Setup the MLX embedding server
 */
export async function setupMlxServer(options = {}) {
    const installDir = options.installDir ?? MLX_HOME;
    const verbose = options.verbose ?? false;
    const log = (...args) => {
        if (verbose)
            console.log('[mlx-setup]', ...args);
    };
    // Find Python 3.10+
    const python = options.pythonPath ?? findPython();
    if (!python) {
        throw new Error('Python 3.10+ required but not found.\n' +
 ...
 */
export function startMlxServer(options = {}) {
    const port = options.port ?? 8000;
    const model = options.model ?? '0.6B';
    const verbose = options.verbose ?? false;
    const serverDir = getMlxServerDir();
    if (!isMlxInstalled()) {
        throw new Error('MLX server not installed. Run: mgrep-local mlx setup');
    }
 ...
 */
export function isMlxServerRunning() {
    if (!existsSync(PID_FILE)) {
        return false;
    }
    try {
 ...
 */
export async function getMlxStatus(serverUrl = 'http://localhost:8000') {
 ...
 */
export async function waitForServer(serverUrl = 'http://localhost:8000', timeoutMs = 60000) {
...
//# sourceMappingURL=mlx-manager.js.map