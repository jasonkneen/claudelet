...
export class ShardedVectorStore {
     ...
     */
    async searchProgressive(embedding, limit = 10, threshold = 0.0, onProgress) {
        this.ensureInitialized();
        ...
        for (const shardId of shardOrder) {
            ...
            const taggedResults = shardResults.map(r => ({
                ...r,
                shardId,
            }));
            ...
            if (onProgress) {
                onProgress(allResults.slice(0, limit), shardId, shardsComplete === this.shardCount);
            }
        }
     ...
     */
    async searchParallel(embedding, limit = 10, threshold = 0.0) {
        this.ensureInitialized();
        // Search all shards in parallel
        const searchPromises = Array.from(this.shards.entries()).map(async ([shardId, shard]) => {
...
//# sourceMappingURL=ShardedVectorStore.js.map