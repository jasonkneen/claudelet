...
async function runWorker() {
    ...
    for (const file of data.files) {
        ...
        try {
            ...
            // Track the file
            await store.trackFile(file.filePath, contentHash, chunks[0]?.metadata.language ?? 'unknown', chunks.length);
            result.filesIndexed++;
    ...
    const finalResult = {
        workerId: data.workerId,
        shardResults: Object.fromEntries(shardResults),
        totalChunks,
        totalFiles,
