 ...
 */
function validateApiKeyResponse(json) {
    if (typeof json !== 'object' || json === null) {
...
export class AnthropicOAuthClient {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('Failed to fetch') || error.message.includes('fetch')) {
                    throw new Error(`${operation} failed: Unable to connect. Check your internet connection.`);
                }
     ...
     */
    async handleHttpError(response, operation) {
        let errorDetails = '';
        try {
     ...
     */
    async parseJsonResponse(response, validator, operation) {
        let json;
        try {
    ...
    async exchangeCodeForTokens(code, verifier, expectedState) {
        ...
        if (callbackState && callbackState !== expectedState) {
            throw new Error('State mismatch: The callback state does not match the expected state. ' +
        ...
        const tokenResponse = await this.parseJsonResponse(response, validateTokenResponse, 'Token exchange');
        return {
            type: 'oauth',
    ...
    async createApiKey(accessToken) {
        ...
        const response = await this.fetchWithTimeout(CREATE_API_KEY_ENDPOINT, {
            method: 'POST',
            headers: {
        ...
        const apiKeyResponse = await this.parseJsonResponse(response, validateApiKeyResponse, 'API key creation');
    ...
    async refreshAccessToken(refreshToken) {
        ...
        const tokenResponse = await this.parseJsonResponse(response, validateTokenResponse, 'Token refresh');
        return {
            type: 'oauth',
            refresh: tokenResponse.refresh_token,
            access: tokenResponse.access_token,
